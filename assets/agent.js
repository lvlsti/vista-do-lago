/* ============================================================
   Vista do Lago — Chat Agent Widget
   Substitui o wa-fab por um agente IA com fallback WhatsApp
   ============================================================ */
(function () {
  ...
  const LABELS = {
    pt: { title: "Assistente Vista do Lago", placeholder: "Como posso ajudar?", send: "Enviar", whatsapp: "Falar no WhatsApp", close: "Fechar", typing: "Digitando...", welcome: "Olá! Sou o assistente da Vista do Lago Jungle Lodge. Posso ajudar com disponibilidade, preços, pacotes e informações sobre a pousada. Como posso ajudar?" },
    en: { title: "Vista do Lago Assistant", placeholder: "How can I help?", send: "Send", whatsapp: "Chat on WhatsApp", close: "Close", typing: "Typing...", welcome: "Hello! I'm the Vista do Lago Jungle Lodge assistant. I can help with availability, prices, packages and information about the lodge. How can I help?" },
    es: { title: "Asistente Vista do Lago", placeholder: "¿En qué puedo ayudar?", send: "Enviar", whatsapp: "Hablar por WhatsApp", close: "Cerrar", typing: "Escribiendo...", welcome: "¡Hola! Soy el asistente de Vista do Lago Jungle Lodge. Puedo ayudarte con disponibilidad, precios, paquetes e información sobre el lodge. ¿En qué puedo ayudarte?" }
  };
  ...
  style.textContent = `
    ...
    .vdl-msg { max-width: 82%; padding: 10px 14px; border-radius: 14px; font-size: 13px; line-height: 1.5; }
  ...
  // --- Threshold scroll (reusar lógica existente) ---
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      hideTyping();
      addMsg(L.whatsapp ? "Ocorreu um erro. Tente pelo WhatsApp." : "An error occurred. Try WhatsApp.", "bot");
    }
    isTyping = false;
  }
  ...
  fab.addEventListener("click", () => {
    ...
    if (isOpen && history.length === 0) {
      setTimeout(() => addMsg(L.welcome, "bot"), 300);
    }
  });

  document.getElementById("vdl-chat-close").addEventListener("click", () => {
    isOpen = false;
    chat.classList.remove("open");
    fab.classList.remove("open");
  });

  document.getElementById("vdl-send").addEventListener("click", () => {
    sendMessage(document.getElementById("vdl-input").value);
  });

  document.getElementById("vdl-input").addEventListener("keydown", e => {
    if (e.key === "Enter") sendMessage(e.target.value);
  });

  // --- Scroll threshold (hero) ---
  let threshold = 0;
  ...
  window.addEventListener("load", () => { calcThreshold(); checkFab(); });
  window.addEventListener("scroll", checkFab, { passive: true });
  window.addEventListener("resize", () => { calcThreshold(); checkFab(); });
